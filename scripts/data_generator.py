...
regions_config = {
    ...
    'EMEA': {'countries': ['United Kingdom', 'Germany', 'France', 'Netherlands'], 'weight': 0.30},
    'APAC': {'countries': ['Japan', 'China', 'Singapore', 'Australia', 'India'], 'weight': 0.25},
...
company_prefixes = ['Alpha', 'Beta', 'Gamma', 'Delta', 'Apex', 'Summit', 'Global', 'Next', 'Prime', 'Elite']
customers_pool = [f"{random.choice(company_prefixes)} {random.choice(company_prefixes)} {random.choice(company_suffixes)}" for _ in range(1000)]

# 分配客戶權重：前 200 個客戶 (VIP) 擁有 80% 的出現機率
customer_weights = np.concatenate([
    np.full(200, 80/200),  # VIP
    np.full(800, 20/800)   # 一般客戶
])
