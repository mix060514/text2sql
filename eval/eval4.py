...
experiment_name = "Text-to-SQL-Comparison-v4"
...
def compare_answers_smart(actual_text, expected, eval_type, eps=1e-2, sql_result=None):
    try:
        ...
        if eval_type == "number":
            ...
            correct_count = 0
            total_items = len(expected_dict) * 2  # key and value

            # We want to check if every key and value in expected exists in the result
            # Ideally they should be paired, but for loose matching, existence is a good start
            # Stronger check: Try to match keys to values

            # Simplified check: Check if all expected values (numbers/strings) are in the flattened result
            # This is usually what matters (e.g. Sales numbers correct for years)
...
def run_comparative_evaluation(agents, eval_set):
    run_name = f"comparison_v4_{int(time.time())}"

    # Output directory
    output_dir = PROJECT_DIR / "eval" / "results"
    output_dir.mkdir(parents=True, exist_ok=True)
    json_path = output_dir / "eval4_comparison.json"
    ...
    with mlflow.start_run(run_name=run_name):
        ...
        # Save detailed CSV
        csv_path = output_dir / "eval4_comparison.csv"
        df.to_csv(csv_path, index=False)
        mlflow.log_artifact(str(csv_path))

        # Save HTML view
        html_path = output_dir / "eval4_comparison.html"
        ...
        prob_csv_path = output_dir / "eval4_problematic.csv"
